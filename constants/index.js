...
const MESSAGES = {
  ...
  INSUFFICIENT_SOL: (minBalance) => `\n❌ Insufficient SOL balance. Minimum required: ${Math.floor(minBalance).toFixed(2)} SOL`,
  TOPUP_SOL: (minBalance) => `\n❌ Your SOL balance is running low. Please arrange a topup: ${Math.floor(minBalance).toFixed(2)} SOL`,
  INSUFFICIENT_TOKEN: (minBalance) => `\n❌ Insufficient ${escapeMarkdown(TOKEN)} balance. Minimum required: ${Math.floor(minBalance).toFixed(2)}`,
