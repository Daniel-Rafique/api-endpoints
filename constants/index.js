...
const MESSAGES = {
  ...
  INSUFFICIENT_SOL: (minBalance) => `\n❌ Insufficient SOL balance. Minimum required: ${minBalance.toFixed(4)} SOL`,
  TOPUP_SOL: (minBalance) => `\n❌ Your SOL balance is running low. Please arrange a topup: ${minBalance.toFixed(4)} SOL`,
  INSUFFICIENT_TOKEN: (minBalance) => `\n❌ Insufficient ${escapeMarkdown(TOKEN)} balance. Minimum required: ${parseFloat(minBalance).toFixed(2)}`,
