...
class Solana {
  ...
  async handleDistribution(chatId, userData) {
    ...
    if (topUpState && updatedBalance > 0) {
      console.log(`topUp in Progress, commissionPaid: ${commissionPaid}, userData.distributeSolana: ${distributeSolana}`);
      ...
      try {
        ...
        const message = MESSAGES.TOPUP_SUCCESS(updatedBalance);
      ...
      // Set commission paid to true and distributeSolana to true to allow for top-ups
      await this.firestore.collection(FIRESTORE_COLLECTION)
        .doc(chatId.toString())
        .update({ distributeSolana: true, commissionPaid: true, topUpState: false });
    }
  }
